...
void Set_Pixel(YUV_IMAGE_T *i, int px, int py, uint8_t lum, uint8_t u,
               uint8_t v) {
...
void YUV_Image_Copy(YUV_IMAGE_T *dst, YUV_IMAGE_T *src) {
  ...
  src->bY, src->w, src->h, (src->w*src->h)); YUV_Image_Info( src);
  YUV_Image_Info( dst);
  */
...
void YUV_Image_Fill(YUV_IMAGE_T *dst, YUV_T *c) {
  // Copy bitplanes
  /* printf("Fill: dst: 0x%x (%d), 0x%x (%d) 0x%x, w: %d h: %d, len:
     %d\n",dst->bY, (dst->bU - dst->bY) ,dst->bU, (dst->bV - dst->bU), dst->bV,
     dst->w, dst->h, dst->w*dst->h); YUV_Image_Info( dst);
  */
...
void YUV_Translate_Image(YUV_IMAGE_T *dst, YUV_IMAGE_T *src, int dX, int dY,
                         float dTheta) {
  ...
  if ((dY != 0) || (dX != 0)) {
    ...
    for (n = 0, y = startY; n < n1; n++, y += incY)
      memcpy(&(dst->bY[y * w + dst1X]), &(src->bY[(y - dY) * w + src1X]),
             n_bytes); // w
    for (n = 0, y = startY / 2; n < n1 / 2; n++, y += incY) {
      memcpy(&(dst->bU[y * half_w + dst1X / 2]),
             &(src->bU[(y - dY / 2) * half_w + src1X / 2]),
             n_bytes / 2); // half_2
      memcpy(&(dst->bV[y * half_w + dst1X / 2]),
             &(src->bV[(y - dY / 2) * half_w + src1X / 2]),
             n_bytes / 2); // half_2
    }
  }
}
...
void Draw_Line_SWD(YUV_IMAGE_T *img, PIXEL_HOLDER_T *pixelInfo, YUV_T *color){
  int pX, pY;
  ...
  XStart = pixelInfo->pxA;
  YStart = pixelInfo->pyA;
  XEnd = pixelInfo->pxB;
  YEnd = pixelInfo->pyB;
  ...
  */
  ...
  if (XDelta >= YDelta) {
    ...
    ** be allocated to either the initial run or last partial run, so we'll add
    *0.5
...
void Draw_Rectangle(YUV_IMAGE_T *i, int pcX, int pcY, int dX, int dY, YUV_T *c,
                    int filled) {
  if (filled > 0) {
    // Not implemented yet
  } else {
...
void Draw_Circle(YUV_IMAGE_T *i, int pcX, int pcY, int radius, YUV_T *c,
                 int filled) {
  int p1X, p1Y, p2X, p2Y;
